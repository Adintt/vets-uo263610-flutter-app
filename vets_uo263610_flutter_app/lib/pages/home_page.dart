...
class StateHomePage extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    final clinics = _clinics.where((clinic) {
      if (_search.isEmpty) {
        return !_showOnlyFavorites || _favoriteClinicIds.contains(clinic.placeId);
      }
      final matchesSearch = clinic.name.toLowerCase().contains(_search) ||
        ...
        children: [
          ...
          Expanded(
            child: Builder(builder: (context) {
              ...
              if (_selectedTab == 0) {
                if (clinics.isEmpty) {
                  return const Center(child: Text('No hay clínicas disponibles'));
                }
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        IconButton(
                                          ...
                                          icon: Icon(
                                            _isFavorite(clinic) ? Icons.favorite : Icons.favorite_border,
                                            color: _isFavorite(clinic) ? Colors.red : null,
                                          ),
                                        ),
                                      ],
                                    ),
                                    Row(children: _buildRatingStars(clinic.rating)),
                                    const SizedBox(height: 6),
                                    Row(
                                      children: [
                                        Icon(
                                          clinic.openNow ? Icons.check_circle : Icons.cancel,
                                          color: clinic.openNow ? Colors.green : Colors.red,
                                        ),
                                        const SizedBox(width: 6),
                                        Text(clinic.openNow ? 'Abierto ahora' : 'Cerrado ahora'),
                                      ],
                                    ),
